...
import stduy.querydsl.dto.MemberDto;
import stduy.querydsl.dto.QMemberDto;
...
public class QuerydslBasicTest {
    ...
    public void join_on_filtering(){
        List<Tuple> result = queryFactory
                ...
                //inner 조인일땐 on절로 값을 가져오나 where 절로 가져오나 값이 같다.
                .fetch();
     ...
     */
    @Test
    public void join_one_no_realation(){
        em.persist(new Member("teamA"));
        em.persist(new Member("teamB"));
        em.persist(new Member("teamC"));
    ...
    //쿼리 dsl 버전
    // getter setter를 통하여 주입 + 기본 생성자 필요
    @Test
...
//                        member.age))
   ...
   @Test
    ...
    public void sqlFunction(){
        List<String> result = queryFactory
                .select(Expressions.stringTemplate("function('replace',{0},{1})", member.username, "M")
                ).from(member)
                .fetch();
        for (String s : result) {
